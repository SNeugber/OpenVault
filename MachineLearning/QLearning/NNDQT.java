...
public class NNDQT {
	...
	private double NN_LEARNRATE = 0.5;
	private double NN_MOMENTUM = 0.7;
	...
	private double biasToFutureReward = 0.8d;
	private double minNoise = 0d;
	...
	public void setMinNoise(double minNoise) {
		if(minNoise < 0 || minNoise > 1) {
			System.err.println("MinNoise for NNDQT out of bounds");
			System.exit(0);
		}
	...
	public void updateTable(int run) {
		...
		epsilon = val;
		if(epsilon < minNoise) epsilon = minNoise;
	...
	private double[] calcLearnVector(double[] state, int action, double[] newState, double reward) {
		...
		oldQVals[action] = this_Q + Q_LEARNRATE * (reward + biasToFutureReward * max_Q - this_Q);
		return oldQVals;
	}
