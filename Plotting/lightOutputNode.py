...
nodeVals=dict()
print netOffsets
for layer in reversed(range(len(netOffsets))):
	if(layer == len(netOffsets)-1):
		for n in range(netOffsets[layer-1]+1,netOffsets[layer]):
			if n == node:
				nodeVals[n]=1
			else:
				nodeVals[n]=0
		...
		start=0
		end=netOffsets[layer]+1
		if layer > 0:
			start=netOffsets[layer-1]+1
		for n in range(start,end):
			thisVal = sum([v['weight']*nodeVals[k] for (k,v) in net[n].iteritems() if k > netOffsets[layer]])
